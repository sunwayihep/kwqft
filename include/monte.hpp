...
MatrixSun<Real, NCOLORS> calculateStaple(
    const Complex<Real>* gauge_ptr,
    int id, int oddbit, int mu, int size,
    ...
    for (int nu = 0; nu < NDIMS; ++nu) {
        ...
        for (int i = 0; i < NCOLORS; ++i) {
            for (int j = 0; j < NCOLORS; ++j) {
                link.e[i][j] = gauge_ptr[idxoddbit + nuvolume + (j + i * NCOLORS) * size];
            }
        }
        ...
        for (int i = 0; i < NCOLORS; ++i) {
            for (int j = 0; j < NCOLORS; ++j) {
                tmp1.e[i][j] = gauge_ptr[newidnu1 + muvolume + (j + i * NCOLORS) * size];
            }
        }
        link = link * tmp1;
        ...
        for (int i = 0; i < NCOLORS; ++i) {
            for (int j = 0; j < NCOLORS; ++j) {
                tmp2.e[i][j] = gauge_ptr[newidmu1 + nuvolume + (j + i * NCOLORS) * size];
            }
        }
        ...
        for (int i = 0; i < NCOLORS; ++i) {
            for (int j = 0; j < NCOLORS; ++j) {
                link.e[i][j] = gauge_ptr[newidnum1 + nuvolume + (j + i * NCOLORS) * size];
            }
        }
        link = link.dagger();
        ...
        for (int i = 0; i < NCOLORS; ++i) {
            for (int j = 0; j < NCOLORS; ++j) {
                tmp1.e[i][j] = gauge_ptr[newidnum1 + muvolume + (j + i * NCOLORS) * size];
            }
        }
        link = link * tmp1;
        ...
        for (int i = 0; i < NCOLORS; ++i) {
            for (int j = 0; j < NCOLORS; ++j) {
                tmp2.e[i][j] = gauge_ptr[newidmun + nuvolume + (j + i * NCOLORS) * size];
            }
        }
        link = link * tmp2;
...
KOKKOS_INLINE_FUNCTION
void heatBathSun(MatrixSun<Real, NCOLORS>& U, 
...
template<typename Real>
KOKKOS_INLINE_FUNCTION
void overrelaxationSun(MatrixSun<Real, NCOLORS>& U,
                       const MatrixSun<Real, NCOLORS>& F)
{
    using MatrixT = MatrixSun<Real, NCOLORS>;
    using ComplexT = Complex<Real>;
...
public:
    ...
    void run() {
        ...
        for (int parity = 0; parity < 2; ++parity) {
            ...
            for (int mu = 0; mu < NDIMS; ++mu) {
                Kokkos::parallel_for("HeatBath",
                    range_policy(0, half_vol),
                    KOKKOS_LAMBDA(const int id) {
                        ...
                        MatrixT staple = calculateStaple<Real>(
                            gauge_ptr, id, parity, mu, size, params);
...
#else
        double phbflop = NCOLORS * NCOLORS * NCOLORS +
            (NCOLORS * (NCOLORS - 1) / 2) * (46LL + 48LL + 56LL * NCOLORS);
        double stapleflop = NCOLORS * NCOLORS * NCOLORS * 84LL;
        long long ThreadFlop = static_cast<long long>((stapleflop + phbflop) * size_);
#endif
    ...
    long long bytes() const {
        ...
        long long bytes_per_site = (20LL * nuparams_ * sizeof(Real) + 2LL * rng_state_size);
...
public:
    ...
    void run() {
        ...
        for (int parity = 0; parity < 2; ++parity) {
            for (int mu = 0; mu < NDIMS; ++mu) {
                Kokkos::parallel_for("Overrelaxation",
                    range_policy(0, half_vol),
                    KOKKOS_LAMBDA(const int id) {
                        ComplexT* gauge_ptr = gauge_view.data();
...
#else
        double ovrflop = NCOLORS * NCOLORS * NCOLORS +
            (NCOLORS * (NCOLORS - 1) / 2) * (46LL + 48LL + 56LL * NCOLORS);
        double stapleflop = NCOLORS * NCOLORS * NCOLORS * 84LL;
        long long ThreadFlop = static_cast<long long>((stapleflop + ovrflop) * params_.half_volume);
#endif
        return ThreadFlop * 2 * NDIMS;
    }
