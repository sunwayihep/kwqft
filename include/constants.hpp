...
struct LatticeParams {
    ...
    LatticeParams() : volume(0), half_volume(0), volume_with_ghost(0),
                      half_volume_with_ghost(0), size(0), kstride(0),
                      tstride(0), beta(0.0), beta_over_nc(0.0),
                      use_texture(false) {
    ...
    // Get grid dimension
    KOKKOS_INLINE_FUNCTION
    int get_grid(int dim) const {
        return grid[dim];
    }

    // Get grid dimension with ghost
    KOKKOS_INLINE_FUNCTION
    int get_grid_g(int dim) const {
        return grid_with_ghost[dim];
    }

    // Get border
    KOKKOS_INLINE_FUNCTION
    int get_border(int dim) const {
        return border[dim];
    }
};
 ...
 */
void initializeParams(const std::vector<int>& lattice_size, double beta, bool verbose = true);
...
KOKKOS_INLINE_FUNCTION
int param_Grid(const LatticeParams& p, int dim) {
    return p.grid[dim];
}
...
KOKKOS_INLINE_FUNCTION
int param_Volume(const LatticeParams& p) {
    return p.volume;
}

KOKKOS_INLINE_FUNCTION
int param_HalfVolume(const LatticeParams& p) {
    return p.half_volume;
}

KOKKOS_INLINE_FUNCTION
int param_VolumeG(const LatticeParams& p) {
    return p.volume_with_ghost;
}
...
KOKKOS_INLINE_FUNCTION
int param_Size(const LatticeParams& p) {
    return p.size;
}

KOKKOS_INLINE_FUNCTION
double param_Beta(const LatticeParams& p) {
    return p.beta;
}

KOKKOS_INLINE_FUNCTION
double param_BetaOverNc(const LatticeParams& p) {
    return p.beta_over_nc;
}

KOKKOS_INLINE_FUNCTION
int param_border(const LatticeParams& p, int dim) {
    return p.border[dim];
}
