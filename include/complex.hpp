 ...
 */
template <typename Real> class Complex {
public:
  Real x; // Real part
  ...
  KOKKOS_INLINE_FUNCTION
  Complex operator+(Real b) const { return Complex(x + b, y); }
  ...
  KOKKOS_INLINE_FUNCTION
  Complex operator*(Real b) const { return Complex(x * b, y * b); }
  ...
  KOKKOS_INLINE_FUNCTION
  Real abs() const { return Kokkos::sqrt(x * x + y * y); }

  // Squared modulus
  KOKKOS_INLINE_FUNCTION
  Real abs2() const { return x * x + y * y; }

  // Phase angle
  KOKKOS_INLINE_FUNCTION
  Real phase() const { return Kokkos::atan2(y, x); }

  KOKKOS_INLINE_FUNCTION
  Real arg() const { return Kokkos::atan2(y, x); }
  ...
  KOKKOS_INLINE_FUNCTION
  static Complex one() { return Complex(Real(1), Real(0)); }
...
KOKKOS_INLINE_FUNCTION Complex<Real> operator+(Real a, const Complex<Real> &z) {
...
template <typename Real>
KOKKOS_INLINE_FUNCTION Complex<Real> operator/(Real a, const Complex<Real> &b) {
  Real denom = Real(1) / (b.x * b.x + b.y * b.y);
...
// Math functions
template <typename Real>
KOKKOS_INLINE_FUNCTION Complex<Real> conj(const Complex<Real> &z) {
  return Complex<Real>(z.x, -z.y);
}

template <typename Real>
KOKKOS_INLINE_FUNCTION Real abs(const Complex<Real> &z) {
  return Kokkos::sqrt(z.x * z.x + z.y * z.y);
}

template <typename Real>
KOKKOS_INLINE_FUNCTION Real abs2(const Complex<Real> &z) {
  return z.x * z.x + z.y * z.y;
}

template <typename Real>
KOKKOS_INLINE_FUNCTION Complex<Real> sqrt(const Complex<Real> &z) {
...
template <typename Real>
KOKKOS_INLINE_FUNCTION Complex<Real> exp(const Complex<Real> &z) {
  Real rho = Kokkos::exp(z.x);
  return Complex<Real>(rho * Kokkos::cos(z.y), rho * Kokkos::sin(z.y));
}

template <typename Real>
KOKKOS_INLINE_FUNCTION Complex<Real> log(const Complex<Real> &z) {
  return Complex<Real>(Kokkos::log(z.abs()), z.arg());
}

template <typename Real>
KOKKOS_INLINE_FUNCTION Complex<Real> polar(Real rho, Real theta) {
  return Complex<Real>(rho * Kokkos::cos(theta), rho * Kokkos::sin(theta));
}

template <typename Real>
KOKKOS_INLINE_FUNCTION Complex<Real> sin(const Complex<Real> &z) {
  return Complex<Real>(Kokkos::sin(z.x) * Kokkos::cosh(z.y),
                       Kokkos::cos(z.x) * Kokkos::sinh(z.y));
}

template <typename Real>
KOKKOS_INLINE_FUNCTION Complex<Real> cos(const Complex<Real> &z) {
