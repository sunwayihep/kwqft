...
void save_gauge_binary(const GaugeArray<Real> &gauge,
  ...
  // Write data
  size_t total_size =
      static_cast<size_t>(size) * num_elems * sizeof(Complex<Real>);
...
template void save_gauge_binary<float>(const GaugeArray<float> &,
                                       const std::string &);
template void save_gauge_binary<double>(const GaugeArray<double> &,
                                        const std::string &);
template void load_gauge_binary<float>(GaugeArray<float> &,
                                       const std::string &);
template void load_gauge_binary<double>(GaugeArray<double> &,
                                        const std::string &);
